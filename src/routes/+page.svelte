<script lang="ts">
    import ZooBanner from '$lib/Zoo.svelte';
    import Tree1 from '$lib/assets/Tree1.svg';
    import Tree2 from '$lib/assets/Tree2.svg';
    import Tree3 from '$lib/assets/Tree3.svg';
    import Tree4 from '$lib/assets/Tree4.svg';
    import IdCard from '$lib/assets/IdCard.svg';

    import { onMount } from 'svelte';


    let chameleonColorIndex = 0;
    let chameleonColor1 = ['#6E8823', '#882325', '#88237E', '#235988'];
    let chameleonColor2 = ['#90AD28', '#AD282A', '#AD28A0', '#286FAD'];
    let chameleonColor3 = ['#8EA628', '#A6282A', '#A62899', '#286BA6'];
    let chameleonColor4 = ['#E3E63B', '#E63B3E', '#E63BD5', '#3B96E6'];
    let chameleonColor5 = ['#B8CC2B', '#CC2B2E', '#CC2BBC', '#2B81CC'];
    let chameleonColor6 = ['#76821B', '#821B1D', '#821B78', '#1B5282'];


    let scrollProgress = 0;
    let eyeOffsetX = 0;
    let eyeOffsetY = 0;
    
    onMount(() => {
        let eyeCenterX = 0;
        let eyeCenterY = 0;

        const eye = document.getElementById('eye-chameleon1');
        if (eye) {
            const rect = eye.getBoundingClientRect();
            eyeCenterX = rect.left + rect.width / 2;
            eyeCenterY = rect.top + rect.height / 2;
        }

        const minX = -3;
        const maxX = 17;
        const minY = -3;
        const maxY = 5;

        function handleScroll() {
            const scrollTop = window.scrollY;
            const docHeight = document.documentElement.scrollHeight - window.innerHeight;
            scrollProgress = docHeight > 0 ? (scrollTop / docHeight) * 100 : 0;
        }

        function handleMouseMove(event: MouseEvent) {
            const mouseX = event.clientX;
            const mouseY = event.clientY;

            let dx = mouseX - eyeCenterX;
            let dy = mouseY - eyeCenterY;

            eyeOffsetX += (Math.max(minX, Math.min(maxX, dx)) - eyeOffsetX) * 0.2;
            eyeOffsetY += (Math.max(minY, Math.min(maxY, dy)) - eyeOffsetY) * 0.2;
        }

        window.addEventListener('scroll', handleScroll);
        window.addEventListener('mousemove', handleMouseMove);
        handleScroll();

        return () => {
            window.removeEventListener('scroll', handleScroll);
            window.removeEventListener('mousemove', handleMouseMove);
        };
    });

    

    let currentSlide = 0;
    const slideCount = 4;

    function syncChameleonColorIndex() {
        chameleonColorIndex = currentSlide;
    }
    function nextSlide() {
        if (currentSlide < slideCount - 1) currentSlide += 1;
        setTimeout(syncChameleonColorIndex, 500);
    }
    function prevSlide() {
        if (currentSlide > 0) currentSlide -= 1;
        setTimeout(syncChameleonColorIndex, 500);
    }
</script>

<style>
    .sky {
        background: #B1F7EB;
    }
    .border {
        background: url("data:image/svg+xml;utf8,<svg width='256' height='128' viewBox='0 0 256 128' fill='none' xmlns='http://www.w3.org/2000/svg'><path d='M128 0C57.3075 0 0 57.3075 0 128H256C256 57.3075 198.692 0 128 0Z' fill='%2376912F'/></svg>") center;
        background-repeat: repeat-x;
        background-size: auto 100%;
        width: 100vw;
        height: 20px;
        transform: translateY(-100%);
        border: none;
    }
</style>


<ZooBanner />

<!-- #083537; -->
<div style="width: 100vw;">
    <h1 class="title" style="z-index: 50; position: sticky; top: 40%; transform: translateY(-50%); width: 100%;">Chameleon</h1>
    
    <div class="button" style="position: fixed; bottom: 0; left: 50%; transform: translate(-50%, 0); margin-bottom: 80px;">
        <svg width="50" height="50" viewBox="0 0 256 256" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M88 145.792L127.5 189M127.5 189L127.5 67M127.5 189L167 145.792" stroke="#ffffffd1" stroke-width="20" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
    </div>

    <div class="sky" style="position: relative; height: 100vh; margin-top: 70vh;">
        <img src={Tree1} alt="Tree 1" style="position: absolute; height: 110%; left: 0; transform: translate(-47%, -7%); z-index:20" />
        <img src={Tree2} alt="Tree 2" style="position: absolute; height: 110%; right: 0; transform: translate(50%, -7%); z-index:30" />
        <img src={Tree3} alt="Tree 3" style="position: absolute; width: 100vw; left: 50%; transform: translate(-50%, 0%); z-index: 0" />
        <img src={Tree4} alt="Tree 4" style="position: absolute; width: 100vw; left: 50%; transform: translate(-50%, -13%); z-index: 60" />
    </div>
</div>

<div style="background-color: #76912f; height: 90vh; position: relative; z-index:100;">
    <div class="border"></div>

    <!-- fake id card for infomation of the chameleon -->
    <div style="display: flex; flex-direction: column; width: 80%; height: 80%; margin: auto; position: relative; top: 50%; transform: translateY(-50%); background: #eaeaea;; border-radius: 15px; box-shadow: 0px 4px 10px rgba(0,0,0,0.1);">
        <h2 style="display: flex; align-items: center; justify-content: center; border-radius: 15px 15px 0 0; width: 100%; height: 42px; background: #002564; text-align: center; color: #fff; font-weight: bold; font-size: 1.15em;">ID CARD</h2>
        <div style="display: flex; flex-direction: row; justify-content: space-evenly; padding: 0px 32px; align-items: center; height: 100%;">
            <div style="width: 30%; height: 100%; display: flex; flex-direction: column; justify-content: center; align-items: center;">
                <img src={IdCard} alt="Chameleon" style="background: #ffffff; width: 80%; border-radius: 20px; box-shadow: 0px 4px 10px rgba(0,0,0,0.1);" />
                <p style="margin-top: 10px; font-weight: bold; font-size: 1.25em;">Chameleon</p>
                <p style="font-size: 1em;">Chamaeleonidae</p>
            </div>
            <div style="width: 55%; height: 100%; display: flex; flex-direction: column; justify-content: center;">
                <span style="display: flex; align-items: flex-start; flex-direction: row; margin-block: 5px;">
                    <p style="font-size: 1.1em; display: flex; align-items: flex-start;">
                        <span class="material-symbols-outlined" style="font-size: 1.5em; color: #161616; margin-right: 10px;">pets</span>
                        Class: Reptilia
                    </p>
                </span>

                <span style="display: flex; align-items: flex-start; flex-direction: row; margin-block: 5px;">
                    <p style="font-size: 1.1em; display: flex; align-items: flex-start;">
                        <span class="material-symbols-outlined" style="font-size: 1.5em; color: #161616; margin-right: 10px;">straighten</span>
                        Size: From 3 cm (Brookesia nana, the smallest) up to 70 cm (Parson's chameleon, the largest)
                    </p>
                </span>

                <span style="display: flex; align-items: flex-start; flex-direction: row; margin-block: 5px;">
                    <p style="font-size: 1.1em; display: flex; align-items: flex-start;">
                        <span class="material-symbols-outlined" style="font-size: 1.5em; color: #161616; margin-right: 10px;">balance</span>
                        Weight: 15g to 200g depending on the species
                    </p>
                </span>

                <span style="display: flex; align-items: flex-start; flex-direction: row; margin-block: 5px;">
                    <p style="font-size: 1.1em; display: flex; align-items: flex-start;">
                        <span class="material-symbols-outlined" style="font-size: 1.5em; color: #161616; margin-right: 10px;">hourglass_empty</span>
                        Lifespan: 5 to 10 years on average
                    </p>
                </span>

                <span style="display: flex; align-items: flex-start; flex-direction: row; margin-block: 5px;">
                    <p style="font-size: 1.1em; display: flex; align-items: flex-start;">
                        <span class="material-symbols-outlined" style="font-size: 1.5em; color: #161616; margin-right: 10px;">park</span>
                        Habitat: Tropical forests, savannas, mountains, and even semi-desert areas
                    </p>
                </span>

                <span style="display: flex; align-items: flex-start; flex-direction: row; margin-block: 5px;">
                    <p style="font-size: 1.1em; display: flex; align-items: flex-start;">
                        <span class="material-symbols-outlined" style="font-size: 1.5em; color: #161616; margin-right: 10px;">public</span>
                        Distribution: Africa, Madagascar (home to most species), southern Europe, Middle East, India, Sri Lanka
                    </p>
                </span>

                <span style="display: flex; align-items: flex-start; flex-direction: row; margin-block: 5px;">
                    <p style="font-size: 1.1em; display: flex; align-items: flex-start;">
                        <span class="material-symbols-outlined" style="font-size: 1.5em; color: #161616; margin-right: 10px;">restaurant</span>
                        Diet: Mostly insects (crickets, flies, grasshoppers), sometimes small birds or lizards
                    </p>
                </span>

                <div style="font-family: 'Libre Barcode 39'; font-size: 3em; text-align: right; height: 50px;">bit.ly/47xw5Zh</div>
            </div>
        </div>
    </div>
</div>

<div style="background: #f4f4f4; height: 90vh; position: relative; z-index:100;">
    <!-- <div class="border"></div> -->

    <!-- <h2></h2> -->

    <div style="transition: transform .65s ease-in-out; width: 100vw; height: 100%; transform: translateX(calc(10% - 25px - (80% + 25px) * {currentSlide})); display: flex; flex-direction: row; flex-wrap: nowrap;">
        <div style="background: #76912F; width: 80%; height: 80%; margin: auto 25px; border-radius: 15px; flex-shrink: 0;"></div>
        <div style="background: #912F2F; width: 80%; height: 80%; margin: auto 25px; border-radius: 15px; flex-shrink: 0;"></div>
        <div style="background: #912F8E; width: 80%; height: 80%; margin: auto 25px; border-radius: 15px; flex-shrink: 0;"></div>
        <div style="background: #2F8491; width: 80%; height: 80%; margin: auto 25px; border-radius: 15px; flex-shrink: 0;"></div>
    </div>


    <div style="position: absolute; bottom: 0; right: 0; transform: rotate(12.36deg) translate(7%, -25%); filter: drop-shadow(-5px 5px 0px rgba(0,0,0,0.25));">
        <svg width="490" height="296" viewBox="0 0 490 296" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path style="transition: fill 1s ease-out;" d="M65.6304 253.03C58.9641 257.763 51.5253 264.862 42.8553 273.106C41.965 273.952 40.985 273.037 41.7951 272.113C50.6569 262.011 60.5925 251.731 61.2686 248.763C49.0391 250.794 20.1489 258.312 23.6075 295.699C65.9256 287.775 65.6304 253.03 65.6304 253.03Z" fill="#7E9B24"/>
            <path style="transition: fill 1s ease-out;" d="M79.7607 223.802C76.9802 209.974 62.8334 183.051 28.4908 185.977C27.8804 199.011 36.222 225.354 74.471 226.456C68.9512 220.615 60.9331 212.799 54.7199 206.937C53.8355 206.102 54.8835 204.735 55.8839 205.426C65.3833 211.99 72.2959 218.423 79.7607 223.802Z" fill="#7E9B24"/>
            <path style="transition: fill 1s ease-out;" d="M302.799 219.893C288.557 222.534 287.54 224.972 286.319 229.117C299.656 229.271 312.805 229.429 325.585 229.59C329.044 223.347 321.11 216.845 321.11 199.247C315.542 200.324 307.781 200.966 299.137 201.241C299.329 209.511 299.912 213.771 302.799 219.893Z" fill={chameleonColor1[chameleonColorIndex]}/>
            <path style="transition: fill 1s ease-out;" d="M218.57 165.235C224.538 173.972 234.236 189.617 234.236 196.526C257.619 200.749 275.386 201.996 299.137 201.241C307.781 200.966 315.542 200.324 321.11 199.247C321.901 199.094 322.648 198.932 323.348 198.761C321.72 193.478 324.243 174.582 343.286 163.203C337.114 165.032 323.795 172.143 319.889 185.96C278.3 193.095 245.425 187.992 218.57 165.235Z" fill={chameleonColor2[chameleonColorIndex]}/>
            <path style="transition: fill 1s ease-out;" d="M148.176 180.474C148.989 180.474 162.01 180.271 168.114 177.63C169.643 178.687 171.857 180.082 174.624 181.656C180.112 184.777 187.776 188.603 196.597 191.904C201.273 193.654 206.274 195.256 211.449 196.526C202.158 185.283 182.968 163.602 181.745 162.797C178.738 160.818 166.594 179.061 148.176 180.474Z" fill={chameleonColor2[chameleonColorIndex]}/>
            <path style="transition: fill 1s ease-out;" d="M148.176 218.267C138.668 222.534 128.034 217.657 122.134 227.828C134.087 227.855 146.628 227.907 159.569 227.98C181.318 214.442 186.911 206.353 196.597 191.904C187.776 188.603 180.112 184.777 174.624 181.656C168.114 196.526 158.145 211.155 148.176 218.267Z" fill={chameleonColor1[chameleonColorIndex]}/>
            <path style="transition: fill 1s ease-out;" d="M479.951 259.735C486.766 260.327 487.783 231.695 478.323 231.695C474.387 231.695 428.316 230.956 366.479 230.123C362.41 235.335 350.814 235.132 346.134 254.289C414.257 256.839 476.828 259.463 479.951 259.735Z" fill="#8A5326"/>
            <path style="transition: fill 1s ease-out;" d="M195.987 229.646C189.661 231.093 187.849 232.042 186.424 241.227C185.808 245.2 183.929 247.071 181.948 247.338C185.529 247.025 187.577 244.353 188.153 243.056C188.05 237.341 189.577 232.764 195.987 229.646Z" fill={chameleonColor3[chameleonColorIndex]}/>
            <path style="transition: fill 1s ease-out;" d="M74.471 226.456C75.4736 227.517 74.8294 229.261 73.377 229.408C49.7621 231.798 11.6347 235.995 6.31379 236.978C0.718222 238.011 -2.42267 244.806 6.31379 249.169C9.67075 250.845 52.2946 246.934 60.2287 247.95C61.2686 248.083 61.4191 248.101 61.2686 248.763C60.5925 251.731 50.6569 262.011 41.7951 272.113C40.985 273.037 41.965 273.952 42.8553 273.106C51.5253 264.862 58.9641 257.763 65.6304 253.03C69.3318 250.402 72.795 248.503 76.0986 247.543C77.6595 247.37 112.488 247.492 137.35 247.91C139.602 247.948 140.714 252.595 138.859 253.873C130.183 259.852 120.58 268.337 119.434 270.3C117.772 273.145 120.248 280.257 124.317 277.209C144.865 254.858 162.362 250.591 170.297 249.575C172.005 249.277 178.639 249.229 188.87 249.37C188.682 248.75 188.542 248.066 188.459 247.323C188.294 245.839 188.177 244.415 188.153 243.056C187.577 244.353 185.529 247.025 181.948 247.338C179.35 247.687 176.578 245.276 176.252 241.227C176.252 235.558 177.674 231.214 180.396 228.111C173.361 228.062 166.409 228.018 159.569 227.98C146.628 227.907 134.087 227.855 122.134 227.828C109.284 227.799 97.1157 227.799 85.8643 227.834C83.7522 226.575 81.7371 225.225 79.7607 223.802C72.2959 218.423 65.3833 211.99 55.8839 205.426C54.8835 204.735 53.8355 206.102 54.7199 206.937C60.9331 212.799 68.9512 220.615 74.471 226.456V226.456Z" fill="#8A5326"/>
            <path style="transition: fill 1s ease-out;" d="M346.134 233.913C340.989 236.765 337.774 237.635 335.758 247.323C335.618 248.259 335.099 249.227 334.37 250.056C335.087 249.244 336.506 247.492 336.728 246.653C337.856 241.695 341 238.42 346.134 233.913Z" fill={chameleonColor3[chameleonColorIndex]}/>
            <path style="transition: fill 1s ease-out;" d="M325.585 229.59C312.805 229.429 299.656 229.271 286.319 229.117C262.5 228.844 238.082 228.59 214.094 228.379C214.094 228.379 213.28 229.442 206.566 233.303C201.204 236.386 200.869 240.008 198.631 248.136C198.494 248.635 198.338 249.1 198.166 249.529C229.98 250.163 284.044 251.989 336.368 253.926C336.138 251.07 336.259 248.716 336.728 246.653C336.506 247.492 335.087 249.244 334.37 250.056L334.334 250.097C331.885 252.84 327.131 254.015 326.196 247.323C325.346 241.242 326.862 234.177 333.486 229.69C330.869 229.656 328.235 229.623 325.585 229.59Z" fill="#8A5326"/>
            <path style="transition: fill 1s ease-out;" d="M249.698 72.1838C258.65 85.5854 261.233 138.801 261.498 140.649C261.764 142.498 264.486 152.003 270.45 141.462C280.451 123.787 279.322 80.7767 273.502 67.763C265.417 68.5338 257.384 70.0159 249.698 72.1838Z" fill={chameleonColor4[chameleonColorIndex]}/>
            <path style="transition: fill 1s ease-out;" d="M204.532 96.5576C211.652 107.53 217.959 133.741 219.18 139.024C220.401 144.307 223.453 146.542 229.963 139.024C236.474 131.506 232.608 90.6651 225.284 82.4455C217.818 86.7862 209.984 92.2014 204.532 96.5576Z" fill={chameleonColor4[chameleonColorIndex]}/>
            <path style="transition: fill 1s ease-out;" d="M314.314 70.2653C311.618 69.6211 308.859 69.0779 306.054 68.6344C307.296 91.0075 303.613 129.962 301.985 138.658C300.357 147.355 305.792 148.251 310.937 141.462C327.701 124.069 330.672 89.9222 330.061 75.0226C324.82 73.0916 319.606 71.5218 314.314 70.2653Z" fill={chameleonColor4[chameleonColorIndex]}/>
            <path style="transition: fill 1s ease-out;" d="M353.662 86.0594C352.026 104.476 345.727 137.398 344.1 141.462C342.472 145.526 343.693 148.574 350 143.697C356.307 138.821 367.687 118.765 373.599 100.012C367.224 94.6419 360.684 90.0875 353.662 86.0594Z" fill={chameleonColor4[chameleonColorIndex]}/>
            <path style="transition: fill 1s ease-out;" fill-rule="evenodd" clip-rule="evenodd" d="M146.545 104.888C146.545 118.13 135.888 128.865 122.741 128.865C109.594 128.865 98.9371 118.13 98.9371 104.888C98.9371 91.6466 109.594 80.9121 122.741 80.9121C135.888 80.9121 146.545 91.6466 146.545 104.888Z" fill="#FEFEFF"/>
            <path style="transition: fill 1s ease-out;" id="eye-chameleon1" transform={`translate(${eyeOffsetX}, ${eyeOffsetY})`} d="M109.951 98.9959C107.388 100.256 105.76 104.888 106.655 108.343C107.55 111.594 111.416 113.707 115.322 113.138C118.74 112.609 120.571 110.171 120.571 106.148C120.571 102.938 119.106 100.54 116.34 99.2397C113.817 98.1019 111.904 98.0206 109.951 98.9959Z" fill="#141311"/>
            <path style="transition: fill 1s ease-out;" d="M133.158 58.4393C119.567 70.0617 115.742 72.2968 101.094 76.9295C100.37 77.1596 99.6508 77.394 98.9371 77.6325C131.427 71.5404 149.422 71.1876 178.5 113.138C184.622 117.419 187.747 115.483 192.973 107.123C195.333 104.604 200.297 89.4052 202.86 76.6856C204.732 67.4609 205.546 60.9589 205.831 52.7501C206.197 42.3062 204.895 36.8607 201.355 33.8536C198.71 31.5779 196.391 31.0089 190.084 31.0496C181.01 31.0902 172.668 33.4065 161.397 38.9333C151.997 43.566 144.388 48.8082 133.158 58.4393Z" fill={chameleonColor6[chameleonColorIndex]}/>
            <path style="transition: fill 1s ease-out;" fill-rule="evenodd" clip-rule="evenodd" d="M181.745 162.797C182.968 163.602 202.158 185.283 211.449 196.526L212.67 197.745C206.948 206.965 203.577 212.224 196.597 222.128C189.301 222.269 183.82 224.208 180.396 228.111C177.674 231.214 176.252 235.558 176.252 241.227C176.578 245.276 179.35 247.687 181.948 247.338C183.929 247.071 185.808 245.2 186.424 241.227C187.849 232.042 189.661 231.093 195.987 229.646C189.577 232.764 188.05 237.341 188.153 243.056C188.177 244.415 188.294 245.839 188.459 247.323C188.542 248.066 188.682 248.75 188.87 249.37C190.516 254.796 195.816 255.416 198.166 249.529C198.338 249.1 198.494 248.635 198.631 248.136C200.869 240.008 201.204 236.386 206.566 233.303C213.28 229.442 214.094 228.379 214.094 228.379C226.097 215.219 234.236 205.669 234.236 196.526C234.236 189.617 224.538 173.972 218.57 165.235C245.425 187.992 278.3 193.095 319.889 185.96C323.795 172.143 337.114 165.032 343.286 163.203C324.243 174.582 321.72 193.478 323.348 198.761C327.417 208.108 331.237 212.309 345.32 225.582C340.248 226.261 336.38 227.73 333.486 229.69C326.862 234.177 325.346 241.242 326.196 247.323C327.131 254.015 331.885 252.84 334.334 250.097L334.37 250.056C335.099 249.227 335.618 248.259 335.758 247.323C337.774 237.635 340.989 236.765 346.134 233.913C341 238.42 337.856 241.695 336.728 246.653C336.259 248.716 336.138 251.07 336.368 253.926C336.606 256.873 341.862 262.562 346.134 254.289C350.814 235.132 362.41 235.335 366.479 230.123C366.933 225.536 364.607 220.976 357.731 210.749C355.086 206.279 353.051 199.167 351.017 197.542C351.424 196.932 351.451 196.742 351.627 196.323C354.272 190.024 371.362 181.49 381.942 171.737C382.552 195.307 424.27 189.049 438.909 181.896C457.766 172.682 461.456 169.005 466.415 162.797C478.907 149.753 486.76 132.035 489.405 111.066C490.056 105.823 490.056 93.1037 489.405 87.9021C486.923 68.0709 479.558 49.9872 468.165 35.6827C460.515 26.0516 449.936 16.5424 441.309 11.5033C434.229 7.35825 422.917 3.17258 414.901 1.75026C412.785 1.38452 408.025 1.01879 404.322 0.937505C394.556 0.693676 388.493 1.79089 381.698 4.96062C361.027 14.7137 350.936 36.0484 357.69 55.6764C359.562 61.1625 362.329 65.592 366.479 69.8183C374.699 78.1897 384.261 82.0909 395.085 81.4813C409.734 80.6686 419.377 71.8502 419.377 59.2525C419.377 52.019 416.122 46.3704 410.222 43.2819C407.943 42.1034 407.496 42.0222 403.508 41.9815C398.503 41.9815 394.353 43.16 392.359 45.1106C391.341 46.1266 391.219 46.4923 391.423 47.9959C391.789 50.678 393.213 51.572 399.846 53.1975C403.467 54.0915 405.949 57.9927 405.339 61.9346C404.973 64.4541 403.63 66.1609 400.985 67.5426C394.963 70.5904 386.58 68.7211 380.233 62.9099C376.815 59.7808 374.78 56.733 373.234 52.3441C371.606 47.7114 371.484 41.5345 372.868 37.0644C375.268 29.4245 383.285 22.6786 392.888 20.1997C396.916 19.1432 406.438 18.7368 411.443 19.387C419.214 20.4436 428.655 24.5073 435.857 29.9528C439.56 32.7568 445.948 39.0962 449.447 43.4445C458.847 55.0668 465.073 69.8183 466.619 84.0415C467.555 92.5348 466.049 107.53 463.486 115.211C459.661 126.63 451.116 137.886 440.333 145.608C432.886 150.972 425.725 153.207 420.354 151.866C417.953 151.256 415.878 148.98 408.309 138.658C396.037 122.004 385.085 109.685 373.599 100.012C367.687 118.765 356.307 138.821 350 143.697C343.693 148.574 342.472 145.526 344.1 141.462C345.727 137.398 352.026 104.476 353.662 86.0594C351.357 84.7375 349.001 83.4722 346.582 82.2534C340.967 79.4185 335.499 77.0261 330.061 75.0226C330.672 89.9222 327.701 124.069 310.937 141.462C305.792 148.251 300.357 147.355 301.985 138.658C303.613 129.962 307.296 91.0075 306.054 68.6344C295.608 66.9828 284.507 66.7138 273.502 67.763C279.322 80.7767 280.451 123.787 270.45 141.462C264.486 152.003 261.764 142.498 261.498 140.649C261.233 138.801 258.65 85.5854 249.698 72.1838C245.133 73.4715 240.69 75.0011 236.433 76.7673C233.093 78.1467 229.24 80.1453 225.284 82.4455C232.608 90.6651 236.474 131.506 229.963 139.024C223.453 146.542 220.401 144.307 219.18 139.024C217.959 133.741 211.652 107.53 204.532 96.5576C199.885 100.508 197.368 102.825 192.973 107.123C187.747 115.483 184.622 117.419 178.5 113.138C149.422 71.1876 131.427 71.5404 98.9371 77.6325C82.7635 83.0371 69.2594 90.5706 59.7116 99.4834C56.375 102.572 52.5501 107.286 51.7363 109.236C51.1666 110.577 51.2073 110.618 52.7942 112C63.9027 121.631 93.6426 132.928 120.742 138.211C124.323 138.902 129.455 139.512 130.065 139.634C129.997 139.663 129.93 139.693 129.862 139.722C129.766 139.764 129.67 139.805 129.573 139.847L129.565 139.851L129.546 139.859L129.426 139.91C129.374 139.933 129.321 139.956 129.269 139.978C126.398 141.21 123.59 142.307 120.742 143.291C107.028 148.032 92.4089 150.168 65.571 152.231C82.0506 168.689 117.044 182.864 148.176 180.474C166.594 179.061 178.738 160.818 181.745 162.797ZM122.741 128.865C135.888 128.865 146.545 118.13 146.545 104.888C146.545 91.6466 135.888 80.9121 122.741 80.9121C109.594 80.9121 98.9371 91.6466 98.9371 104.888C98.9371 118.13 109.594 128.865 122.741 128.865Z" fill={chameleonColor5[chameleonColorIndex]}/>
        </svg>
    </div>
    

    <!-- <span style="position: absolute; bottom: 0; margin: 25px; font-size: 1.25em; font-weight: bold; color: #ffffffde;">
        {(currentSlide + 1).toString().padStart(2, '0')} / {slideCount.toString().padStart(2, '0')}
    </span> -->

    <div style="position: absolute; bottom: 0; display: flex; flex-direction: row; align-items: center; justify-content: center; width: 100%; gap: 125px; margin-bottom: 50px;">
        <button class="button" style="transform: rotate(90deg);" on:click={prevSlide} aria-label="Back">
            <svg width="50" height="50" viewBox="0 0 256 256" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M88 145.792L127.5 189M127.5 189L127.5 67M127.5 189L167 145.792" stroke="#ffffffd1" stroke-width="20" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
        </button>
        <button class="button" style="transform: rotate(-90deg);" on:click={nextSlide} aria-label="Next">
            <svg width="50" height="50" viewBox="0 0 256 256" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M88 145.792L127.5 189M127.5 189L127.5 67M127.5 189L167 145.792" stroke="#ffffffd1" stroke-width="20" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
        </button>
    </div>
</div>

<div style="background-color: #f48954; height: 90vh; position: relative; z-index:100;">
    <!-- <div class="border"></div> -->
</div>

<div style="background-color: #DDE9BB; height: 90vh; position: relative; z-index:100;">
    <!-- <div class="border"></div> -->
</div>

<div style="background-color: #f48954; height: 90vh; position: relative; z-index:100;">
    <!-- <div class="border"></div> -->
</div>


<div style="background-color: #76912F; height: 250px; position: relative; z-index:100;">
    <div class="border"></div>
</div>

<!-- <div style="width: 100vw; height: 10px; position: sticky; bottom: 0; z-index: 200">
    <div style="background: #00000096; width: 100vw; height: 10px; bottom: 0; z-index: 200">
        <div style="position: absolute; width: {scrollProgress}%; height: 10px; background: #334517; z-index: 210"></div>
    </div>
</div> -->
