<script lang="ts">
    ...
    function autoChameleonAttack() {
        ...
        const chance = isDay ? chameleonLuckDay : chameleonLuckNight;
    ...
    function initFly() {
        ...
        lastFlyX = Math.random() * (zoneRect.width - flyWidth);

        fly.style.left = `${lastFlyX}px`;
    }
    ...
    function moveFly() {
        ...
        if (!isPauseGame) {
            ...
            fly.style.transition = `left ${speedFly}s ease-in-out, top ${speedFly}s ease-in-out`;
            
            const deltaX = flyX - lastFlyX;
            if (Math.abs(deltaX) > 10) {
                fly.style.transform = deltaX < 0 ? "scaleX(1)" : "scaleX(-1)";
            }
        ...
        setTimeout(moveFly, Math.random() * 1000 + 500);
    }
